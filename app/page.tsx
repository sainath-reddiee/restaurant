    ...
    <div className="min-h-screen bg-[#f2f4f7] pb-24 font-sans">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {itemCount > 0 && (
                <Button 
                  size="sm" 
                  className="bg-black text-white hover:bg-gray-800 rounded-full px-4 shadow-xl shadow-orange-500/20 transition-transform hover:scale-105 active:scale-95"
                  onClick={() => {
        ...
        <div className="relative z-10 container mx-auto text-center max-w-2xl mt-4">
          ...
          <div className="relative max-w-lg mx-auto group mt-8">
            ...
            <div className="relative flex items-center bg-white rounded-full p-2 shadow-2xl transform transition-transform group-hover:scale-[1.01]">
              ...
              />
              <Button className="rounded-full px-6 h-10 bg-black hover:bg-gray-900 text-white font-bold text-sm shadow-lg">
                Search
              </Button>
            </div>
          </div>
                    ...
                    {liveLootItems.slice(0, 2).map(item => (
                      <div key={item.id} onClick={() => router.push(`/r/${item.restaurants.slug}`)} className="bg-white/10 backdrop-blur-md p-3.5 rounded-2xl flex items-center justify-between hover:bg-white/20 transition-all cursor-pointer border border-white/5">
                        ...
                        <div className="text-right">
                          ...
                          <span className="block text-orange-200 text-[10px] line-through font-medium">{formatPrice(item.base_price)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-10">
              {filteredRestaurants.map((restaurant, index) => (
                <Card
                  key={restaurant.id}
                  className="group border-0 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 overflow-hidden bg-white rounded-[1.5rem] cursor-pointer ring-1 ring-gray-100"
                ...
                >
                  <div className="relative h-52 overflow-hidden">
                    ...
                    <div className="absolute bottom-4 left-4 right-4 text-white">
                      ...
                      <div className="flex items-center gap-2 text-xs font-medium">
                        <div className="bg-green-500 text-white px-1.5 py-0.5 rounded-md flex items-center gap-0.5 shadow-sm">
                          <span className="text-[10px]">★</span> 4.2
                        </div>
                        <span className="opacity-80">• 30-40 mins • ₹{restaurant.delivery_fee}</span>
                      </div>
                    </div>

                    <div className="absolute top-4 right-4">
                       <Badge className="bg-white/90 text-green-700 backdrop-blur-md shadow-lg border-0 px-2.5 py-0.5 font-bold hover:bg-white text-xs">
                         OPEN
                       </Badge>
                    </div>
                  </div>
                  ...
                  <div className="p-4 flex items-center justify-between bg-white">
                     ...
                     {restaurant.free_delivery_threshold && (
                        <div className="flex items-center gap-1 text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-full border border-blue-100">
      ...
      <Dialog open={isLocationOpen} onOpenChange={setIsLocationOpen}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-center pb-2">Select Location</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <Button 
              variant="outline" 
              className="w-full justify-start h-12 text-orange-600 border-orange-100 bg-orange-50/50 hover:bg-orange-50 font-bold"
              onClick={() => {
                detectLocation();
                setIsLocationOpen(false);
              }}
            ...
            <div className="relative py-2">
              ...
              <div className="relative flex justify-center text-xs uppercase tracking-widest">
                <span className="bg-background px-2 text-gray-400">Or type manually</span>
              </div>
            </div>
